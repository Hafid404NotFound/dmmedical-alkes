...
export default function ExperimentViewer360({
  imageCount,
  onExperimentComplete,
}: ExperimentViewer360Props) {
  ...
  useEffect(() => {
    const preloadImages = async () => {
      ...
      console.log(
        `Loaded ${imageCount} images in ${(loadEndTime - loadStartTime).toFixed(
          2
        )}ms`
      );
    };
    ...
    <div className="w-full max-w-2xl mx-auto">
      ...
      <div className="mb-4 md:mb-6 p-3 md:p-4 bg-blue-50 rounded-lg">
        <h3 className="text-base md:text-lg font-semibold mb-2">
          Experiment: {imageCount} Gambar 360°
        </h3>
        <p className="text-xs md:text-sm text-gray-600 mb-2">
          Menggunakan gambar dari folder:{" "}
          <code className="bg-gray-200 px-1 rounded text-xs">
            /gambar 360/{imageCount}gambar/
          </code>
        </p>
        ...
        <div className="mt-3 md:mt-4 text-center px-2">
          <p className="text-xs md:text-sm text-gray-600 mb-3 md:mb-4">
            ...
            <span className="hidden md:inline">
              Drag atau swipe untuk memutar gambar 360°
            </span>
            <br className="md:hidden" />
            <span className="text-xs">
              Rasakan smoothness dan responsivitas.
            </span>
          </p>
          <Button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 md:p-4">
          <div className="bg-white rounded-lg w-full max-w-sm md:max-w-md max-h-[90vh] overflow-y-auto p-4 md:p-6 mx-2">
            <h3 className="text-base md:text-lg font-semibold mb-3 md:mb-4">
              Rating Experiment ({imageCount} Gambar)
            </h3>
            <RatingStars
